...
public class StringUtils  extends org.apache.commons.lang3.StringUtils {
	...
	/**
	 * 检测对应文本是不是以制定prefix开头.
	 * @param prefix 前缀
	 * @param text  要检测的文本
	 * @return 如果文本以prefix开头，返回true，否则返回false
	 */
	public static boolean startWith(String text, String prefix) {
		String trimmedTest = text.trim();
		int prefixLen = prefix.length();
		if(trimmedTest != null && trimmedTest.length() > prefixLen ) {
			if(prefix.equalsIgnoreCase(trimmedTest.substring(0, prefixLen))) {
				return true;
			}
		}
		return false;
